...
public class CBNGramFeature {
	...
	private void addValues(int valueOffset, int base, int del) {
		int ind = this.dat.get(base).base + del;
		if (ind >= this.datSize || this.dat.get(ind).check != base) return;
		int offset = this.dat.get(ind).base;
	...
	private Vector<Integer> findBases(int datSize, int ch1, int ch2) {
		...
		if (ch1 >= datSize || this.dat.get(ch1).check != 0) {
			uniBase = -1;
			biBase = -1;
			result.clear();
			result.add(uniBase);
		...
		uniBase = this.dat.get(ch1).base + this.separator;
		int ind = this.dat.get(ch1).base + ch2;
		if (ind >= datSize || this.dat.get(ind).check != ch1) {
			biBase = -1;
			result.clear();
			result.add(uniBase);
			result.add(biBase);
		...
		biBase = this.dat.get(ind).base + this.separator;
		result.clear();
		result.add(uniBase);
		result.add(biBase);
		return result;
