...
public class Preprocesser {
	...
	public Preprocesser() {
		this.otherSet = new HashSet<>();
		this.singlePunSet = new HashSet<>();
		this.httpSet = new HashSet<>();
		...
		len = 41;
		for (int i = 0; i < len; i++) {
			this.singlePunSet.add(singlePun[i]);
		}

		char httpChar[] = {'/', '.', ':', '#', '"', '_', '-', '=', '+', '&', '$', ';'};
		len = 12;
		for (int i = 0; i < len; i++) {
			this.httpSet.add((int) httpChar[i]);
		}
	}

	public boolean isOther(int c) {
		if (this.otherSet.contains(c)) {
			return true;
		} else {
			return false;
		}
	...
	public boolean isSinglePun(int c) {
		if (this.singlePunSet.contains(c)) {
			return true;
		} else {
			return false;
		}
	...
	public void setT2SMap(String filename) throws IOException {
		...
		for (int i = 0; i < datSize; i++) {
			this.t2s.put(tra[i], sim[i]);
			this.s2t.put(sim[i], tra[i]);
		}
		in.close();
	}
	...
	public String clean(String sentence, POCGraph graph) {
		...
		int titleStart = -1;
		Vector<Integer> titleStartVec = new Vector<>();
		Vector<Raw> titleVec = new Vector<>();
...
//	            System.out.print(c+" ");
	...
	int getT2S(int c) {
		if (this.t2s.containsKey(c)) {
			return this.t2s.get(c);
		} else {
			return c;
		}
	}
	...
	int getS2T(int c) {
		if (this.s2t.containsKey(c)) {
			return this.s2t.get(c);
		} else {
			return c;
		}
	}
	...
	boolean containsT(String sentence) {
		for (int i = 0; i < sentence.length(); i++) {
			if (this.t2s.containsKey(sentence.charAt(i))) {
				return true;
			}
		}
		return false;
	...
	void S2T(List<TaggedWord> sentence, String oriSentence) {
		int count = 0;
		for (int i = 0; i < sentence.size(); i++) {
			for (int j = 0; j < sentence.get(i).word.length(); j++) {
				sentence.get(i).word = sentence.get(i).word.substring(0,
						j - 1) + oriSentence.charAt(count)
